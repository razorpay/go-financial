...
func (r *Reducing) GetPrincipal(config Config, period int64) decimal.Decimal {
	return PPmt(config.getInterestRatePerPeriodInDecimal(), period, config.periods, config.AmountBorrowed, 0, config.PaymentPeriod)
}
...
func (r *Reducing) GetInterest(config Config, period int64) decimal.Decimal {
	return *IPmt(config.getInterestRatePerPeriodInDecimal(), period, config.periods, config.AmountBorrowed, 0, config.PaymentPeriod)
}
...
func (r *Reducing) GetPayment(config Config) decimal.Decimal {
	return Pmt(config.getInterestRatePerPeriodInDecimal(), config.periods, config.AmountBorrowed, 0, config.PaymentPeriod)
}
